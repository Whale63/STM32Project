...
int IR_Posture_distinction(void)//if the ir sensor reads LINES,it gives digital output '0',else it returns '1'
{
	...
	if((sensor[0]==1)&&(sensor[1]==1&&sensor[2]==1)&&(sensor[3]==1)&&(Distance()==0))//if the posture is correct
		{		
			posture=3;
			PWM_SetCompare(70);
//			Delay_ms(delay1);
...
//			Delay_ms(delay1);
	...
	else if((sensor[0]==1)&&(sensor[1]==1)&&(sensor[2]==0)&&(sensor[3]==0)&&(Distance()==0))
		{
...
//			Delay_ms(delay1);
	...
	else if((sensor[0]==0)&&(sensor[1]==0)&&(sensor[2]==1)&&(sensor[3]==1)&&(Distance()==0))
		{
			posture=22;//2 means very left
			PWM_SetCompare(50);
//			Delay_ms(delay1);
		}
	else if((sensor[0]==1)&&(sensor[1]==0)&&(sensor[2]==0)&&(sensor[3]==0)&&(Distance()==0))
		{
			posture=111;//1 means  right
			PWM_SetCompare(50);
		}
	else if((sensor[0]==0)&&(sensor[1]==0)&&(sensor[2]==0)&&(sensor[3]==1)&&(Distance()==0))
		{
			posture=222;//1 means  left
			PWM_SetCompare(50);
		}
	else if((sensor[0]==0)&&(sensor[1]==1&&sensor[2]==1)&&(sensor[3]==0)&&(Distance()==1))//if the posture is correct
		{		
			posture=6;
			PWM_SetCompare(50);
		}
