...
int OUTPUT1,OUTPUT2,OUTPUT3,OUTPUT4;// OUTPUT1 和OUTPUT2 是控制小车前左右轮速度的PWM输出值，OUTPUT3，OUTPUT4是左后右后

int PID_value=0;//姿态PID控制器的PWM输出值
...
{  
	 float Position_KP=160,Position_KD=90; //参数整定，需要调到适合自己小车的数值大小  //因为没有用到KI(积分)，所以这里去除了和KI相关的式子//160,90
...
void Su(void)//将OUTPUT1，OUTPUT2，OUTPUT3，OUTPUT4输出出去，使舵机和小车电机受到相应控制
{
		  
			TIM_SetCompare1(TIM2,OUTPUT3); //前电机控制
			TIM_SetCompare2(TIM2,OUTPUT4);//前电机控制
	    TIM_SetCompare3(TIM4,OUTPUT1);//后电机控制
			TIM_SetCompare4(TIM4,OUTPUT2);//后电机控制
...
{	
	  ...
	  OUTPUT2=PID_jc-PID_sd;//基础速度-速度偏差量
		OUTPUT3=PID_jc+PID_sd;//基础速度+速度偏差量 //基础速度也可以恒定为某个值，这样调差速和舵机就比较好调
	  OUTPUT4=PID_jc-PID_sd;//基础速度-速度偏差量
    ...
    if(OUTPUT2<0) OUTPUT2=0;
		if(OUTPUT3>7200) OUTPUT2=7200;
    if(OUTPUT3<0) OUTPUT2=0;
		if(OUTPUT4>7200) OUTPUT2=7200;
		if(OUTPUT4<0) OUTPUT2=0;
