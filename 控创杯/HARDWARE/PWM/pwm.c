...
{  
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);// 
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);  //使能GPIO外设时钟使能，开启完全复用功能到PB10.11
                                                       	
   //设置该引脚为复用输出功能,输出TIM4_CH3的PWM脉冲波形
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8; //管脚对应默认复用功能为TIM4_CH3(定时器四的通道三)
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;  //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化GPIOB8
	
	 //设置该引脚为复用输出功能,输出TIM4_CH4的PWM脉冲波形
	...
	TIM_OC3Init(TIM4, &TIM_OCInitStructure);  //根据TIM_OCInitStruct中指定的参数初始化TIM4_CH3
  ...
  TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1; //选择定时器模式:TIM脉冲宽度调制模式1
	...
	TIM_OCInitStructure.TIM_Pulse =0; //设置待装入捕获比较寄存器的脉冲值
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; //输出极性:TIM输出比较极性高 
	TIM_OC4Init(TIM4, &TIM_OCInitStructure); //根据TIM_OCInitStruct中指定的参数初始化TIM4_CH4
  ...
  TIM_Cmd(TIM4, ENABLE);  //开启TIM4时钟
	
}//初始化控制后轮子运动状态的pwm输出引脚
